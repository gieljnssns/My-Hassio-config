...
def ask(session_cookie, model, query, token_budget=4096 - 500):
    ...
    _LOGGER.debug("Dumping url %r", url)
    _LOGGER.debug("Dumping data %r", data)
    _LOGGER.debug("Dumping headers %r", headers)
    _LOGGER.debug("Dumping cookie %r", cookies)
    _LOGGER.debug("Dumping json response %r", response)
