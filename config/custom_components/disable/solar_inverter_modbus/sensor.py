...
from .const import (
    ...
    SOLAR_INVERTER_MODBUS_DOMAIN,
)

from .huawei_const import HUAWEI_DEVICE_STATUS_DEFINITIONS, HUAWEI_GRID_CODES

from .solaredge_const import SOLAREDGE_DEVICE_STATUSSES
...
async def async_setup_platform(
    ...
    for entry in discovery_info[CONF_SENSORS]:
        ...
        if CONF_SCAN_INTERVAL not in entry:
            if not discovery_info[CONF_SCAN_INTERVAL]:
                entry[CONF_SCAN_INTERVAL] = DEFAULT_SCAN_INTERVAL
            else:
                entry[CONF_SCAN_INTERVAL] = discovery_info[CONF_SCAN_INTERVAL]

        hub: ModbusHub = hass.data[SOLAR_INVERTER_MODBUS_DOMAIN][
            discovery_info[CONF_BRAND]
        ]
        sensors.append(
...
class ModbusRegisterSensor(RestoreEntity, SensorEntity):
    ...
    def __init__(
        ...
        self._scan_interval = datetime.timedelta(seconds=scan_interval)
        self._type = type
    ...
    def state(self):
        ...
        if self._type in ["startup_time", "shutdown_time", "system_time"]:
            try:
                ...
                return self._value
        elif self._type == "device_status" and self._brand == "solaredge":
            return SOLAREDGE_DEVICE_STATUSSES.get(self._value)
        elif self._type == "device_status" and self._brand == "huawei":
            return HUAWEI_DEVICE_STATUS_DEFINITIONS.get(self._value)
        elif self._type == "grid_code" and self._brand == "huawei":
            return HUAWEI_GRID_CODES.get(self._value)
    ...
    def name(self):
        ...
        return f"{self._brand.capitalize()} {self._name}"
