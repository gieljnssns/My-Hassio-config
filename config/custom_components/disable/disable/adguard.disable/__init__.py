...
# from . import AdGuardHome, AdGuardHomeConnectionError, AdGuardHomeError
...
from .const import (
    ...
    DOMAIN,
    SERVICE_ADD_URL,
    SERVICE_DISABLE_CLIENT,
    SERVICE_DISABLE_URL,
    SERVICE_ENABLE_CLIENT,
    SERVICE_ENABLE_URL,
    SERVICE_REFRESH,
    SERVICE_REMOVE_URL,
)
...
SERVICE_CLIENT_SCHEMA = vol.Schema({vol.Required(CONF_IP_ADDRESS): cv.string})
SERVICE_DISABLE_CLIENT_SCHEMA = vol.Schema({vol.Required(CONF_IP_ADDRESS): cv.string})
SERVICE_ENABLE_CLIENT_SCHEMA = vol.Schema({vol.Required(CONF_IP_ADDRESS): cv.string})
...
async def async_setup_entry(hass: HomeAssistantType, entry: ConfigEntry) -> bool:
    ...
    async def enable_client(call) -> None:
        """Service call to enable a client in AdGuard Home."""
        await adguard.client.enable_client(call.data.get(CONF_IP_ADDRESS))

    async def disable_client(call) -> None:
        """Service call to disable a client in AdGuard Home."""
        await adguard.client.disable_client(call.data.get(CONF_IP_ADDRESS))
    ...
    hass.services.async_register(
        DOMAIN, SERVICE_ENABLE_CLIENT, enable_client, schema=SERVICE_CLIENT_SCHEMA
    )
    hass.services.async_register(
        DOMAIN, SERVICE_DISABLE_CLIENT, disable_client, schema=SERVICE_CLIENT_SCHEMA
    )
    hass.services.async_register(
...
async def async_unload_entry(hass: HomeAssistantType, entry: ConfigType) -> bool:
    ...
    hass.services.async_remove(DOMAIN, SERVICE_ENABLE_CLIENT)
    hass.services.async_remove(DOMAIN, SERVICE_DISABLE_CLIENT)
...
class AdGuardHomeEntity(Entity):
    ...
    async def async_update(self) -> None:
        ...
        try:
            ...
            self._available = True
        except adguardhome.AdGuardHomeError:
            if self._available:
                _LOGGER.debug(
