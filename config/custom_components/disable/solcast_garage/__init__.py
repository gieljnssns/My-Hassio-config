...
async def async_setup(hass, config):
    ...
    rooftop_site = SolcastRooftopSite(
        hass,
        api_key,
        resource_id,
        api_limit,
        disable_ssl,
        disable_auto_forecast_fetching,
        disable_auto_history_fetching,
    )
...
class SolcastRooftopSite(SolcastAPI):
    ...
    def start_periodic_update(self):
        ...
        async_track_utc_time_change(
            self._hass, self.reset_api_limit, hour=0, minute=0, second=0, local=True
        )
    ...
    async def _fetch_forecasts(self) -> bool:
        ...
        resp = await self.request_data(
            f"/rooftop_sites/{self._resource_id}/forecasts", ssl=not self._disable_ssl
        )
    ...
    async def _post_single_measurement(self, data):
        ...
        return await self.post_data(
            f"/rooftop_sites/{self._resource_id}/measurements",
            data,
            ssl=not self._disable_ssl,
        )
