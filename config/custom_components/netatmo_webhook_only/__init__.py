...
DOMAIN = 'netatmo_webhook_only'

SERVICE_ADDWEBHOOK2 = 'addwebhook2'
SERVICE_DROPWEBHOOK2 = 'dropwebhook2'

NETATMO_WEBHOOK_ONLY_AUTH = None
...
SCHEMA_SERVICE_ADDWEBHOOK2 = vol.Schema({
    vol.Optional(CONF_URL): cv.string,
})

SCHEMA_SERVICE_DROPWEBHOOK2 = vol.Schema({})
...
def setup(hass, config):
    ...
    global NETATMO_WEBHOOK_ONLY_AUTH
    ...
    if config[DOMAIN][CONF_WEBHOOKS]:
        ...
        NETATMO_AUTH.addwebhook2(hass.data[DATA_WEBHOOK_URL])
        hass.bus.listen_once(
            EVENT_HOMEASSISTANT_STOP, dropwebhook2)

    def _service_addwebhook2(service):
        ...
        NETATMO_AUTH.addwebhook2(url)

    hass.services.register(
        DOMAIN, SERVICE_ADDWEBHOOK2, _service_addwebhook2,
        schema=SCHEMA_SERVICE_ADDWEBHOOK2)

    def _service_dropwebhook2(service):
        """Service to drop webhooks during runtime."""
        _LOGGER.info("Dropping webhook")
        NETATMO_AUTH.dropwebhook2()

    hass.services.register(
        DOMAIN, SERVICE_DROPWEBHOOK2, _service_dropwebhook2,
        schema=SCHEMA_SERVICE_DROPWEBHOOK2)

    return True


def dropwebhook2(hass):
    """Drop the webhook subscription."""
    NETATMO_WEBHOOK_ONLY_AUTH.dropwebhook2()
