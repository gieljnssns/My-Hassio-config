...
class SmappeeSensor(Entity):
    ...
    def update(self):
        ...
        data1 = self._smappee.get_consumption(self._location_id, aggregation=1, delta=30)
        data2 = self._smappee.get_consumption(self._location_id, aggregation=3, delta=1440)
        data3 = self._smappee.load_instantaneous()
        if SENSOR_TYPES[self._sensor][0] == 'Always On':
            ...
            consumption = data1.get('consumptions')[-1]
            ...
            value1 = [float(i['value']) for i in data3 if i['key'].endswith('phase0ActivePower')]
            value2 = [float(i['value']) for i in data3 if i['key'].endswith('phase1ActivePower')]
            value3 = [float(i['value']) for i in data3 if i['key'].endswith('phase2ActivePower')]
            ...
            value1 = [float(i['value']) for i in data3 if i['key'].endswith('phase3ActivePower')]
            value2 = [float(i['value']) for i in data3 if i['key'].endswith('phase4ActivePower')]
            value3 = [float(i['value']) for i in data3 if i['key'].endswith('phase5ActivePower')]
            current = sum(value1 + value2 + value3) / 1000
            if current < 0:
                current = 0
            self._state = round(current, 2)
