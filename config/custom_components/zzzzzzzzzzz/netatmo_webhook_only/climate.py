...
DEPENDENCIES = ['netatmo_webhook_only']
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    netatmo = hass.components.netatmo_webhook_only

    import pyatmo
    homes_conf = config.get(CONF_HOMES)
    try:
        home_data = HomeData(netatmo.NETATMO_WEBHOOK_ONLY_AUTH)
    ...
    for home in homes:
        ...
        try:
            room_data = ThermostatData(netatmo.NETATMO_WEBHOOK_ONLY_AUTH, home)
        except pyatmo.NoDevice:
            continue
...
class NetatmoThermostat(ClimateDevice):
    ...
    def __init__(self, data, room_id):
        ...
        self._name = 'netatmo_webhook_only{}'.format(room_name)
